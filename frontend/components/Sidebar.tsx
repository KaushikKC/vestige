    ...
    <>
      ...
      `}>
        ...
        <div className="p-8 pb-8 flex-shrink-0">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-[0_0_15px_rgba(255,255,255,0.3)]">
              <div className="w-0 h-0 border-l-[6px] border-l-transparent border-b-[10px] border-b-[#1D04E1] border-r-[6px] border-r-transparent mb-0.5" />
            </div>
        ...
        <div className="flex-1 px-0 py-4 flex flex-col gap-1 no-scrollbar overflow-y-auto">
          <div className="px-8 mb-4 text-xs font-bold text-white/40 uppercase tracking-widest">Menu</div>
          
          {menuItems.map((item, index) => {
            // Logic to simulate 'Discover' being the active one for visual matching
            const isActive = (item.label === 'Discover' && currentView === ViewState.DISCOVER) || 
            ...
            return (
              <div key={index} className="relative group w-full">
                ...
                >
                  <item.icon 
                    size={20} 
                    className={isActive ? 'text-[#1D04E1]' : 'text-white/70 group-hover:text-white'} 
