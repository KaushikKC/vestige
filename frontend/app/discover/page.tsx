...
// Map on-chain LaunchData to UI Launch type
function launchDataToLaunch(data: LaunchData): Launch {
  const launchPda = data.publicKey.toBase58();
  const progress =
    data.graduationTarget.toNumber() > 0
      ? Math.min(
          100,
          (data.totalCommitted.toNumber() / data.graduationTarget.toNumber()) *
            100,
        )
      : 0;
  ...
  return {
    ...
    symbol: "VEST",
    status: data.isGraduated ? "GRADUATED" : "PRIVATE",
    ...
    totalCommitted: data.totalCommitted.toNumber(),
    totalParticipants: data.totalParticipants.toNumber(),
    isDelegated: data.isDelegated,
    minCommitment: data.minCommitment.toNumber(),
    maxCommitment: data.maxCommitment.toNumber(),
    color: data.isDelegated ? "#B19DDC" : "#1D04E1",
  };
}
    ...
    <div className="space-y-8 animate-fade-slide-up pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row h-full">
          ...
          <div className="p-10 md:p-14 md:w-3/5 flex flex-col justify-center">
            ...
            <p className="text-gray-300 max-w-md text-sm md:text-base font-medium mb-8">
              The first privacy-preserving launchpad on Solana. Participate in
              price discovery without revealing your hand.
            </p>
            ...
            <div className="relative w-64 h-80 perspective-1000">
              ...
              <div className="absolute inset-0 bg-[#CFEA4D] rounded-[24px] shadow-[0_0_50px_rgba(207,234,77,0.2)] transform rotate-[6deg] translate-x-[20px] z-20 flex flex-col p-5 text-[#09090A] transition-transform hover:rotate-[8deg] hover:translate-x-[25px] duration-500">
                <div className="flex justify-between items-start mb-2">
                  <div className="w-10 h-10 bg-black/10 rounded-full flex items-center justify-center font-bold">
                    N
                  </div>
                  <span className="bg-black/10 px-2 py-1 rounded text-xs font-bold">
                    +145%
                  </span>
                </div>
      ...
      <div className="bg-white rounded-2xl p-6 border-2 border-[#CFEA4D] shadow-sm">
        ...
        <p className="text-sm text-[#6B7280] mb-4">
          Paste your Launch PDA (e.g. from Create Launch success) to go directly
          to that launch.
        </p>
        <div className="flex gap-3 flex-wrap">
          <input
            ...
            placeholder="Paste Launch PDA (e.g. AQhWGegUFpzSPc3antDE6PMRcevY3gxEVcTHt9Xa5tNZ)"
            className="flex-1 min-w-[200px] px-4 py-3 bg-[#F5F6FA] border-2 border-[#E6E8EF] rounded-xl focus:ring-2 focus:ring-[#3A2BFF] focus:border-[#3A2BFF] outline-none text-sm font-mono"
          />
          <button
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sticky top-0 z-20 py-2 bg-[#FDFCFB]/80 backdrop-blur-sm -mx-4 px-4">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          {["All Assets", "Privacy", "DeFi", "Gaming", "Infra"].map(
            (cat, i) => (
              <button
                key={cat}
                className={`
              ...
              <div className="absolute top-full right-0 mt-2 w-40 bg-white rounded-2xl shadow-xl border border-[#E6E8EF] overflow-hidden z-30 animate-[fadeIn_0.2s_ease-out]">
                {["1h", "24h", "7d", "30d", "All time"].map((t) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 text-sm font-medium hover:bg-[#F5F6FA] transition-colors ${
                      timeRange === t
                        ? "text-[#1D04E1] bg-[#F5F6FA]"
                        : "text-[#09090A]"
                    }`}
                  >
                    {t}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          {/* Featured Launches - Real on-chain launches */}
          <div className="space-y-4">
            ...
            <div className="flex gap-5 overflow-x-auto pb-6 -mx-4 px-4 scroll-smooth no-scrollbar snap-x">
              {realLaunches.length === 0 && !loadingLaunches && (
                <div className="min-w-[280px] bg-[#F5F6FA] rounded-2xl p-6 border-2 border-dashed border-[#E6E8EF] text-center">
                  <p className="text-[#6B7280] font-medium mb-2">
                    No on-chain launches yet
                  </p>
                  <p className="text-sm text-[#6B7280] mb-4">
              ...
              {realLaunches.map((launch) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      <div
                        className="w-12 h-12 rounded-2xl bg-[#F5F6FA] flex items-center justify-center font-bold text-xl group-hover:scale-110 transition-transform shadow-inner text-[#09090A]"
                      ...
                      <div>
                        ...
                        <div className="text-xs font-semibold text-[#6B7280] flex items-center gap-1">
                          {launch.symbol}
                          {launch.isDelegated && (
                            <Lock size={10} className="text-[#B19DDC]" />
                          )}
                        </div>
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-[#F5F6FA] rounded-[32px] p-8 min-h-[240px] relative overflow-hidden group cursor-pointer border border-[#E6E8EF] hover:border-[#B19DDC] transition-all hover:shadow-lg">
              <div className="relative z-10 flex flex-col h-full">
                <span className="bg-[#B19DDC] text-white px-3 py-1 rounded-full text-[10px] font-bold w-fit mb-3 uppercase tracking-wide">
                  Insight
                </span>
            ...
            <div className="bg-[#1D04E1] rounded-[32px] p-8 min-h-[240px] relative overflow-hidden group cursor-pointer border border-transparent hover:border-[#CFEA4D] transition-all hover:shadow-lg">
              <div className="relative z-10 flex flex-col h-full text-white">
                <span className="bg-[#CFEA4D] text-[#09090A] px-3 py-1 rounded-full text-[10px] font-bold w-fit mb-3 uppercase tracking-wide">
                  Ecosystem
                </span>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-[24px] border border-[#E6E8EF] shadow-sm overflow-hidden">
            {[
              ...
              <div
                key={i}
                className="flex items-center justify-between p-5 border-b border-[#F5F6FA] last:border-0 hover:bg-[#F9FAFB] cursor-pointer transition-colors group"
              >
                <div className="flex items-center gap-3 w-1/3">
                  ...
                  <div>
                    <div className="font-bold text-[#09090A] text-sm">
                      {coin.sym}
                    </div>
