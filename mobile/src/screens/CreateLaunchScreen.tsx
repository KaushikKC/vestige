...
// ─── Root Screen ───────────────────────────────────────────────────────────────
// Owns ONLY createdPda. Does not own any form state → never re-renders on typing.
...
const LaunchForm = memo(function LaunchForm({
  ...
  // formKey increments only when test mode is toggled.
  // Only the numeric FormFields (which have key={...-formKey}) will remount.
  // Identity fields have no key — they NEVER remount and always read defaultValue="".
  const [formKey, setFormKey] = useState(0);

  // All field values live in refs. Typing → ref mutation only → ZERO re-renders.
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      await initializeLaunch(
        mintKeypair, supply, bonus, startTime, endTime,
        pMaxVal, pMinVal, rBestVal, rMinVal, target,
        tokenNameRef.current || 'Vestige Token',
        tokenSymbolRef.current || 'VSTG',
        tokenUriRef.current || '',
      );
  ...
  return (
    <>
      ...
      {/* Identity: isolated in IdentityFields so LaunchForm re-renders never touch these inputs */}
      <IdentityFields
...
const FormField = memo(function FormField({
  ...
  const handleChangeText = useCallback(
    (text: string) => { valueRef.current = text; },
    [valueRef],
  );
